...
public class RabbitMQConfiguration {
    ...
    //create 4 key for fund (Routing key)
    //آدرس روی پاکت نامه
...
//other
    ...
    Queue reviewUpdatedQueue(){
        return new Queue(REVIEW_UPDATED_QUEUE);
    }
    @Bean
    Queue reviewDeletedQueue(){
        return new Queue(REVIEW_DELETED_QUEUE);
    }

    //اتصال Queue به Exchange با Routing Key (Binding)
    @Bean
    Binding reviewCreatedBinding(final  Queue reviewCreatedQueue,final DirectExchange reviewExchange){
        return BindingBuilder
                .bind(reviewCreatedQueue)// این صندوق
                .to(reviewExchange)// به این مرکز توزیع
                .with(REVIEW_CREATED_KEY);// فقط اگر آدرس نامه "review.created" باشد
    }
    @Bean
